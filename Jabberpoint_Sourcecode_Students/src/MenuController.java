...
public class MenuController extends MenuBar {
	...
	private void openPresentation() {
		...
		try {
			XMLFIleSelector XFS = new XMLFIleSelector(); // Assuming the correct name is 'XMLFileSelector'
			String selectedFile = XFS.selectXmlFile(); // get the selected file path
			if (selectedFile != null) { // Check if a file was selected
				xmlAccessor.loadFile(presentation, selectedFile);
				presentation.setSlideNumber(0);
			} else {
				// Handle the case where the user cancelled the file selection
				JOptionPane.showMessageDialog(parent, "File selection was cancelled.", "Open Cancelled", JOptionPane.INFORMATION_MESSAGE);
			}
